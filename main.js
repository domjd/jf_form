...
function showResults(val) {
  ...
  let list = "";
  fetch(
    `https://sea-lion-app-lccwh.ondigitalocean.app/companies/getCompaniesMain?companyname=${val}`,
    ...
    .then(function (data) {
      for (let i = 0; i < 3; i++) {
        list +=
          `<li class="company" onclick=setCompany(${i})>` +
          data.items[i].title +
          " | " +
          data.items[i].company_number +
          '<div class="companyAddress">' +
          data.items[i].address_snippet +
          "</div>" +
          "</li>";
      }
...
function setCompany(index) {
  ...
  addressTwo =
    selectedCompany.address.address_line_2 == null
      ? selectedCompany.address.locality
      : selectedCompany.address.address_line_2;
  addressThree =
    selectedCompany.address.region == null
      ? selectedCompany.address.country == null
        ? ""
...
window.onload = function () {
  ...
  const submitForm = async () => {
    ...
    const companyUrl =
      selectedCompany.links && selectedCompany.links.self
        ? "https://find-and-update.company-information.service.gov.uk" +
          selectedCompany.links.self
        : "";
    ...
    const formData = {
      ...
      confirmations: [
        document.getElementById("bankaccount").checked,
    ...
    try {
      const response = await fetch(
        "https://sea-lion-app-lccwh.ondigitalocean.app/jotform/submitminiform",
        {
